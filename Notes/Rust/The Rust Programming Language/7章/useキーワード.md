...
**「関数をインポートする場合」と「構造体や`Enum`をインポートする場合」**で推奨される「短縮の仕方」が異なる

### 関数の場合: 親モジュールまでを`use`する

```rust
use crate::front_of_house::hosting; // 親(hosting)まで指定

hosting::add_to_waitlist(); // 呼び出すときに「hosting::」をつける
```

- 関数をインポートする場合は、親モジュールまでを`use`で指定し、呼び出すときにモジュール名を付けることで、関数の所属が明確になる

### 構造体や`Enum`の場合: それ自体を`use`する

```rust
use std::collections::HashMap; // 本体まで指定
